<template>
  <Dialog v-model:visible="planDialog" :breakpoints="{'500px': '100vw' ,'960px': '75vw', '5000px':'512px'}"
    ...
    <div class="body-dialog-plans">
      <div class="head smart-text-align">
        ...
        {{ formatPriceNumber(plan.price, plan.currencyId) }}
            ...
            <swiper-slide v-for="(account, index) in state.accounts" :key="index" class="pt-2 d-flex w-100 px-1">
              <Button class="p-button-text item-get-account p-0" type="button" @click="selectedAccount(account, index)">
                ...
                <div class="footer-item">
                  <span>{{ formatPriceNumber(account.originalCash, account.currencyId) }}</span>
  ...
  <chargeAccount
      v-if="chargeAccountDialog"
      :show="chargeAccount"
      :account-id="selectedAccountNumber"
  />
</template>
...
import chargeAccount from "@/app/unit/wallet/walletTransaction/chargeAccount.vue";
...
export default {
  ...
  emits: ["done"],
  components: {currency, Swiper, SwiperSlide,chargeAccount},
  setup(props, {emit}) {
    ...
    function selectedAccount(obj, index) {
      ...
      selectedAccountNumber.value = obj.accountNumber
      selectStyle.value = index;
      emit("done", selectedAccountInfo.value);
    }


    const chargeAccount = () => {
      chargeAccountDialog.value = true
    };

    function sendInfo() {
      let model = {
        productId: route.value.params.productId,
        immediatePlanId: props.plansId,
        depositorPageId: Math.abs(getSelectedPageId()),
        cityId: getSelectedPageData().cityId,
        depositorAccountNumber: selectedAccountInfo.value.accountNumber,
    ...
    return {
      ...
      sendInfo,
      password,
      chargeAccountDialog,selectedAccountNumber
    };
  },
};
</script>
