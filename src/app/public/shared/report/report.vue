  ...
  <div class="report-bar">
    <div class="bar-content">
      <div class="input-report">
        <Textarea v-model="inputVal"  id="input" class="input-bar p-1 px-2 w-100" :autoResize="true" rows="1" cols="10" :placeholder="$t('description')"  />
<!--      <input-->
<!--        type="text"-->
<!--        v-model="inputVal"-->
<!--        :placeholder="$t('description')"-->
<!--        class="input-bar p-1 px-2 w-100"-->
<!--        id="input"-->
<!--      />-->
      </div>
...
export default {
  components: { ratio , Textarea},
  props:['userData', 'itemType', 'itemId','propId'],
  setup(props, {emit}) {
    const otherPhotoList = ref([]);
    const btn1 = ref(i18n.global.tc("inappropriatePhoto"));
    const inputVal = ref("");
    const state = ref(new reportState());
    function reportApi() {
      let model = {
        pageId: getSelectedPageId(),
        itemType: Number(props.itemType) ,
        itemId: props.itemId,
        propId: props.propId,
        title: "گزارش اشکال",
...
.input-bar {
  ...
  text-align: center;
  border: 1px solid rgb(170, 167, 167) !important;
  border-radius: 7px;
}
