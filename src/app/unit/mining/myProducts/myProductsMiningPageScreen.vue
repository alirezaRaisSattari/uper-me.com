    ...
    <div v-if="state.posts.length !== 0">
        ...
        <div class="box-item-controller">
          <div class="btn-controller">
              <template v-if="state.miningDetails">
                <template v-if="state.selectedItem.productId !== state.miningDetails.productId">
                    <p>{{$t('otherItemMining')}}</p>
                </template>
                <template v-else-if="state.selectedItem.productId == state.miningDetails.productId">
                  <p>{{$t('Extracting')}}</p>
                </template>
              </template>
              <template v-else>
                    <Button :label="$t('mine')" class="p-button-rounded no-shadow" @click="state.createMining(state.selectedItem.productId)"/>
              </template>
          </div>
          <div v-ripple class="item p-ripple">
            <div class="controller">
              ...
              <div class="child">
                  <span class="smart-text-align">{{ $filters.jalali(state.selectedItem.modifiedAt , true) }}</span>
              <span class="smart-text-align">{{$t('notMinedView', {index: state.selectedItem.numberOfViews})}}</span>
              <span>{{$t('product')}} : </span>
      ...
      <div v-if="state.time === null && state.onFinish && state.miningDetails" class="box-mining-main-controller">
        <div class="box-controller">
          <Button :label="$t('depositToAccount')" class="p-button-rounded no-shadow" @click="state.mineTransaction(state.miningDetails.productId)"/>
      ...
      <div v-if="state.miningDetails && state.miningDetails.mainImageUrl" class="box-mining-item">
        <div class="child" v-if="state.miningDetails">
          <img v-lazy="state.miningDetails.mainImageUrl+'_sm.webp'" alt="Cover">
        </div>
        <div class="child">
          <span
      ...
      <div class="list-controller">
        <div v-for="item in state.posts" :key="item" v-ripple class="item p-ripple"
          ...
          <div class="controller">
            ...
            <div class="child">
              <span class="smart-text-align">{{ $filters.jalali(item.modifiedAt , true) }}</span>
              <span class="smart-text-align">{{$t('notMinedView', {index: item.numberOfViews})}}</span>
              <span>{{$t('product')}} : </span>
              <span>{{item.name}}</span>
            </div>
          </div>
...
import {myProductsMiningState} from './myProductsMiningPageState'
...
export default {
  ...
  setup() {
    const state = ref(new myProductsMiningState());
