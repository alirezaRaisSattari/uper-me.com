<template>
  <div>
    <div v-if="state.posts.length !== 0">
        ...
        <div class="box-item-controller">
          ...
          <div v-ripple class="item p-ripple">
            <div class="controller">
              ...
              <div class="child">
                 <span class="reverse-smart-text-align">{{ $filters.jalali(state.selectedItem.createdAt , true) }}</span>
                  <span class="smart-text-align">{{$t('notMinedLike', {index: state.selectedItem.numberOfLikes})}}</span>
                  <span>{{state.selectedItem.caption}}</span>
                  <span>{{$t('comment')}} : {{state.selectedItem.text}}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="state.miningDetails && state.miningDetails.postId" class="box-mining-item">
      ...
      <div class="list-controller">
        <div v-for="item in state.posts" :key="item" v-ripple class="item p-ripple"
          ...
          <div class="controller">
            ...
            <div class="child">
              <span class="reverse-smart-text-align">{{ $filters.jalali(item.createdAt , true) }}</span>
              <span class="smart-text-align">{{$t('notMinedLike', {index: item.numberOfLikes})}}</span>
              <span>{{item.caption}}</span>
              <span>{{$t('comment')}} : {{item.text}}</span>
            </div>
          </div>
        </div>
...
import {pageCommentMiningPageState} from './pageCommentMiningPageState'
...
export default {
  ...
  setup() {
    const state = ref(new pageCommentMiningPageState());
