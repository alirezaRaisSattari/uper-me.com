			...
			<div class="item" v-for="item in list" :key="item">
				...
				<div class="middel">
					<div class="child text-right">
						...
						<span>{{$t('totalPrice')}} : <span>{{$filters.currency(item.price , item.currencyId == 0)}}</span>
						<currency class="mx-2" :currencyId="item.currencyId"></currency>
						</span>
					</div>
					<div class="child text-left">
						<span>{{$t('status')}} : <span>{{item.customStatus == 1 ? $t('notCheck'):item.customStatus == 2 ? $t('accepted'):$t('notAccepted')}}</span></span>
						<span>{{$t('count')}} : <span>{{item.number}}</span></span>
					</div>
				</div>
				<div class="footer" @click="$function.goTo(`/customShopMadeProductPerview/${item.customId}`)">
					<div class="child">
...
export default {
	...
	setup () {
		...
		onMounted(() => {
			loading.value = true;
			ProductRepository.getInstance().GetCustomsForBuyer({
				body:{
					skip: 0,
					take: 1000,
					workPageId:  Number(route.value.params.wid),
