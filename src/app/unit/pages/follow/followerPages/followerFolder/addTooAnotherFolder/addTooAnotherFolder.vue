<template>
  <div class="container">
    ...
    >
      ...
      <div class="hover-pointer" @click="displayCreateNewFolderDialog = true">
    ...
    <div v-else id="scroll-follow-folder" class="row">
      <div
          v-for="item in state.followerFolderList"
          :key="item"
          class="
          col-5
          p-1
      ...
      <div v-if="!state.repliesIsFinished" class="col-12 flex-column mx-auto">
        <loading/>
      </div>
    </div>
  </div>
    ...
    const transferToAnotherFolderFunc = (targetFolderId) => {
      if (pagesId.length == 0) {
        ToastService.getInstance().error(
            i18n.global.tc("not selected follower")
        );
        return;
      }
      ...
      let model = {
        ...
        sourceFolderId: parseInt(props.folderId),
        targetFolderId: targetFolderId,
      };
      state.value.moveFollowers(model);
