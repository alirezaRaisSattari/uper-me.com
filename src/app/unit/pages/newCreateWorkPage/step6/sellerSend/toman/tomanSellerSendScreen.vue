      ...
      <Form v-slot="{ errors }" :validation-schema="addressSchema"
        ...
        <div class="row mx-0 mt-0 mb-0 justify-content-center ">
          <div class="col-12 col-sm-8">
            ...
            <Field id="phone_Title" v-model="toman.newPhone.title" :class="{ 'is-invalid': errors.phone_Title }"
          ...
          <div class="col-12 col-sm-8">
            ...
            <Field id="phone_number" v-model="toman.newPhone.phone" :class="{ 'is-invalid': errors.phone_number }"
    ...
    <div class="row m-0 mt-4">
      <div class="col-12">
        <Form v-slot="{ errors }" :validation-schema="addressSchemaMain"
          ...
          <Field id="textAddress" v-model="toman.postDataTomanSeller.textAddress"
                 :class="{ 'is-invalid': errors.textAddress }"
                 :placeholder="$t('addressAndDescription')" class="form-control my-2"
          ...
          <div class="row">
            <div class="col-9 col-md-10 pl-1 mt-2">
              <label v-if="toman.list.length > 0" class="my-2 w-100 text-right" for="phones">{{ $t("phones") }}</label>
              <swiper
                  id="phones"
                  ...
                  class="slider"
              >
                <swiper-slide v-for="(phone , index) in toman.list" :key="index" class="pt-2 col-8 col-md-5 pr-0 pl-2">
                  <createItemList :index="index" :phone="phone" class="my-3"
                                  @delete="(event)=> toman.deletePhone(event)"
                                  @edit="(event) =>toman.editDetails(event)"/>
                </swiper-slide>
              </swiper>
            </div>
            ...
            <div class="col-12">
              <div class="d-flex">
                ...
                <Checkbox v-model="toman.postDataTomanSeller.itsFree" :binary="true"/>
              </div>

              <div v-show="toman.postDataTomanSeller.itsFree === false ">
                <hr>

                <div class="p-field p-col-12 my-2">
                  <label class="w-100 smart-text-align" for="sendRange">{{ $t('pricePerHundredValue') }}</label>
                  <InputNumber id="sendRange" v-model="toman.postDataTomanSeller.sendRange"
                ...
                <div class="p-field p-col-12 my-2">
                  ...
                  <InputNumber id="sendPrice" v-model="toman.postDataTomanSeller.sendPrice"
...
import {tomanSellerSendState} from './tomanSellerSendState';
...
export default {
  ...
  setup(props, {emit}) {
    ...
    const toman = ref(new tomanSellerSendState((newItem) => {
    ...
    onMounted(() => {
      toman.value.pushData(props.data);
    });


    function getCurrentLatlng(location) {
      mapExist.value = false;
      toman.value.getLocationByLatLon(location);
    }
    ...
    const addressSchemaMain = Yup.object().shape({
      ...
      sendRange: Yup.string()
          .required('sendRange is required')
          .test('len', i18n.global.tc('inputValidation', {
            min: 0,
            max: 10
      ...
      sendPrice: Yup.string()
          .required('sendPrice is required')
          .test('len', i18n.global.tc('inputValidation', {
            min: 0,
            max: 10
          }), val => (val.length >= 0 && val.length <= 10))
    ...
    function onSubmitAddress(values) {
      emit('sendData', {step: 1, postData: toman.value.postDataTomanSeller});
    }
