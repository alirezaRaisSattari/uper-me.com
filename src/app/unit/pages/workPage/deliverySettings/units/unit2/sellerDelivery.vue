<template>
  ...
  <div>
    <div class="d-flex justify-content-center">
      <Dropdown
        class="w-50 mx-auto text-center mt-3 mb-2"
    ...
    >
      ...
      >
        <div class="d-flex justify-content-between">
          <p class="my-auto mr-4 mt-1">{{ $t("isFree") }}</p>
          <div
            class="
              mt-4
              d-flex
        ...
        >
          <Field
            id="limit"
            :class="errors.limit ? 'border border-danger' : ''"
            :placeholder="$t('pricePerHundredValue')"
            type="number"
            v-model="state.deliveryList.sellerSendFreePerMeter"
            class="form-control border-radius-10px px-2 py-3 inputs"
            name="limit"
          />
          <div class="text-danger text-sm text-right w-100 my-2 px-2">
          ...
          <Field
            id="cost"
            type="number"
            :class="errors.cost ? 'border border-danger' : ''"
            :placeholder="$t('sellerSendFreeKilo')"
            v-model="state.deliveryList.pricePerHunderedMeter"
            class="form-control border-radius-10px px-2 py-3 inputs"
            name="cost"
          />
          <div class="text-danger text-sm text-right w-100 my-2 px-2">
...
<script>
import { sellerDeliveryState } from "./sellerDeliveryState";
...
export default {
  ...
  setup() {
    const state = ref(new sellerDeliveryState());
    ...
    const pageNameSchema = Yup.object().shape({
      ...
      limit: Yup.string()
        .test(
          "len",
          i18n.global.tc("inputValidation", { min: 1, max: 80 }),
          (val) => val.length >= 1 && val.length <= 80
        )
        .default(""),
      cost: Yup.string()
        .test(
