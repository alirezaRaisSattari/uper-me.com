...
<template>
  <div>
    <p v-if="state.posts.length === 0 && !state.loading" class="text-center">
      {{ $t("noPosts") }}
    </p>

    <!--posts-->
    <div class="m-0 p-0 mt-1 row">
      <div
        v-for="post in state.posts"
        :key="post"
        class="col-4 m-0 p-0 hover-pointer"
...
import { WorkPagePostsState } from "./WorkPagePostsState";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const state = ref(new WorkPagePostsState());
    ...
    const toDisplayPost = (postId) => {
      router.push(
        `/post/wp/${Math.abs(pageId.value)}/${postId}/${
