<template>
  <Dialog
    ...
    :breakpoints="{ '500px': '100vw', '960px': '75vw', '5000px': '512px' }"
    :contentStyle="{
      ...
      <div class="body">
        <div class="head-body">
          ...
          <Button
            ...
            class="btn-get-account p-button-raised p-button-text p-button-plain"
            @click="chargeAccount()"
          />
        </div>
        ...
        >
          ...
          >
            <swiper-slide
              ...
              :key="index"
              class="pt-2 d-flex w-100 px-1"
            >
              <Button
                class="p-button-text item-get-account p-0"
                type="button"
              ...
              >
                <div class="header-item smart-text-align">
                  <span>
                    <i
                      :class="selectStyle === index ? 'pi pi-check-circle' : ''"
                    ></i>
                    {{ account.title }}
                  </span>
                </div>
...
import { landerAccountDialogState } from "./landerAccountDialogState";
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const state = ref(new landerAccountDialogState());
    const selectedAccountInfo = ref(null);
    const showDialog = ref(props.planDialog);
    const password = ref("");
    ...
    function sendInfo() {
      let model = {
        planId: Number(props.plan.id),
        countryId: Number(route.value.query.countryId),
        categoryId: Number(route.value.query.catId),
