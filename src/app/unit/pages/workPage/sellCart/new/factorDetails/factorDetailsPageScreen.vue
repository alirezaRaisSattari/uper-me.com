<template>
	<div class="pb-5">
		...
		<div v-if="state.factor && state.factor.factor.factorStatus !== 9">
			<div class="d-flex flex-column ">
				<div class="controller"  v-for="(item,index) in state.dataList" :key="item">
					<template v-if="item.value" >
						<span class="smart-text-align">{{item.label}} :</span>
						<div class="d-flex" v-if="index == 2">
							<span>{{$filters.currency(item.value)}}</span>
							<Currency :currencyId="0" class="mx-1"/>
						</div>
						<div class="d-flex" v-else-if="index == 5">
							<span>{{$filters.currency(item.value)}}</span>
					...
					<div class="accspted-controller" :class="preparationShow ? 'active':null">
						<template  v-if="!preparationShow">
							...
							<div class="child smart-text-align" >
								<img v-lazy="state.factor.logs[0].profileImageUrl+'_sm.webp'" alt="">
			...
			<div class="transferee" v-if="state.factor && state.factor.factor.buyerAddressObject">
					<FactorDetailsMidlleItem v-if="state.factor" :factor="state.factor"/>
			</div>
			<div class="list-contoller">
				<div class="controller w-100" :class="state.factor.factor.productType == 2 || state.factor.factor.productType == 3 || state.factor.factor.productType == 4 ? '':'mt-0'" v-for="item in state.factor.details" :key="item">
					<div class="header" v-if="(state.factor.factor.productType == 2 || state.factor.factor.productType == 3 || state.factor.factor.productType == 4) ">
					...
					<div class="text-controller">
						<div class="child name">
							<span>{{item.name ?? item.customObject.name}}</span>
						</div>
						<div class="child">
							<div class="off" v-if="item.offPercent > 0">
								<span>{{item.offPercent}} %</span>
								<span>{{state.factor.factor.currencyId == 0 ? $filters.currency(item.uperPrice) : item.defaultPrice ? $filters.currency(item.defaultPrice) : $filters.currency(item.customObject.price)}}
									<Currency :currencyId="state.factor.factor.currencyId" class="mx-1"/>
								</span>
							</div>
							<div class="d-flex flex-row" v-else>
								<span>{{state.factor.factor.currencyId == 0 ? $filters.currency(item.uperPrice) : item.defaultPrice ? $filters.currency(item.defaultPrice) : $filters.currency(item.customObject.price)}}</span>
								<Currency :currencyId="state.factor.factor.currencyId" class="mx-1"/>
							</div>
						</div>
						<div class="child">
		...
		<div v-if="state.factor && state.factor.factor.factorStatus == 9 && state.retrunedData !== null">
				<div class="d-flex flex-column ">
				<div class="controller"  v-for="(item,index) in state.dataList" :key="item">
					<span class="smart-text-align">{{item.label}} :</span>
					<div class="d-flex" v-if="index == 2">
						<span v-if="state.factor.factor.factorStatus == 9">{{$filters.currency(state.count)}}</span>
						<span v-else>{{$filters.currency(item.value)}}</span>
        				<Currency v-if="state.factor.factor.factorStatus !== 9" :currencyId="0" class="mx-1"/>
					</div>
					<div class="d-flex" v-else-if="index == 5">
						<span>{{$filters.currency(item.value)}}</span>
						<Currency :currencyId="state.factor.factor.currencyId" class="mx-1"/>
					</div>
					<div class="d-flex" v-else-if="index == 3 && state.factor.factor.factorStatus == 9">
							<span>{{$filters.currency(item.value)}}</span>
							<Currency  :currencyId="state.factor.factor.currencyId" class="mx-1"/>
					</div>
					<span v-else>{{item.value}} </span>
			...
			<div v-if="state.factor  &&  (state.factor.factor.factorStatus === 10 || state.factor.factor.factorStatus === 11)">
				<div class="cancel-controller">
					<div class="btn-canceledByYou-controller">
							<Button :label=" state.factor.factor.factorStatus === 10 ? $t('canceledByYou'):$t('canceledByBuyer') " class="p-button-rounded p-button-danger" />
					</div>
...
import {factorDetailsPageState} from './factorDetailsPageState'
