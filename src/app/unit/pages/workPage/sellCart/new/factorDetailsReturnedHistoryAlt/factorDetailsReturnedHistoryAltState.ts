...
export class factorDetailsPageState {
	...
	count = 0;
	getData() {
		FactorRepository.getInstance().ReturnedFactorGetByIdForBuyer({
			urlContent:`${this.route.params.factorId}/${(getSelectedPageId())}`
	...
	calcReturned() {
		...
		this.retrunedData.items.forEach(item => {
			sum +=item.currencyId == 0 ? item.uperPrice * item.number ?? 0 - (((item.uperPrice * item.number ?? 0) / 100) * (item.offPercent ?? 0))  :  ((item.defaultPrice * item.number) - (((item.defaultPrice * item.number) / 100) * (item.offPercent ?? 0)))
		});
