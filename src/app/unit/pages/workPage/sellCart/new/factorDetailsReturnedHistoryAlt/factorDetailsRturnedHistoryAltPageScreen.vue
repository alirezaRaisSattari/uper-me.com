	...
	<div class="pb-5">
		...
		<div v-if=" state.retrunedData !== null">
			...
			<div class="list-contoller">
				<div class="controller w-100 mt-2" v-for="item in state.retrunedData.items" :key="item" :class="item.number && item.factorNumbers ? null :'mb-0'">
					<div class="child">
					...
					<div class="footer-controller" >
						<CustomFooter v-if="item.operatorStatus == 1 && item.number && item.factorNumbers" :bg="'#f6596a'" :title="$t('checking')" />
						<CustomFooter v-else-if="item.operatorStatus == 2" :bg="'#fea200'" :title="$t('cancelByOperator')" />
						<CustomFooter v-else-if="item.operatorStatus == 3" :bg="'#a7ff96'" :title="$t('approvedByTheOperator')" />
					</div>
				</div>
			</div>
		</div>
...
import {factorDetailsPageState} from './factorDetailsReturnedHistoryAltState'
