<template>
  ...
  <div class="d-flex justify-content-center container">
    <div class="get-image-wrap bg-light  row">
      ...
      <div class="col-12 mt-3 d-flex px-1 mx-0 justify-content-between">  <!--madarek-->
        <div class="hide-input">
          <input
              ref="documentInput" accept="image/*" multiple type="file" @change="onFileChangeDocuments($event, 2)"/>
        </div>
        <div class="col-4"> {{ $t("documentsImages") }}</div>
        <div class="col-8 reverse-smart-text-align px-0 mx-0">
          <div class="other-pics mx-2 " @click="pushPhotoDocuments(2)">
            {{ $t("otherPhoto") }}
          </div>
        </div>
      </div>
      ...
      <div class="row smart-text-align px-0 mx-0 w-100">
        <div v-for="item in allDocuments" :key="item" class="col-4 my-2">
          <imageItem :item="item" :type="2" @DeleteImage="(event) => DeleteImageDocument(event)"/>
        </div>
...
export default {
  ...
  setup() {
    ...
    function pushPhotoDocuments() {
      documentInput.value.click();
    }

    function onFileChangeDocuments(e, params) {
      if (e.target.files.length === 0) return;
      ...
      if (e.target.files.length + otherPhotoFiles.value.length <= 18) {
        for (let i = 0; i < e.target.files.length; i++) {
          convertFileTOBase64(e.target.files[i], (base64) => {
            allDocuments.value.push({file: base64});
          })
        }
      } else {
        ToastService.getInstance().error(i18n.global.tc("Max18"));
    ...
    function DeleteImageDocument(file, i) {
      const list = [];
      for (let x = 0; x < allDocuments.value.length; x++) {
        if (file !== allDocuments.value[x].file) {
          list.push(allDocuments.value[x]);
        }
      }
      allDocuments.value = list;
    }
