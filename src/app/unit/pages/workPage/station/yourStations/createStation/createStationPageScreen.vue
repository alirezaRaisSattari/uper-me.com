          ...
          <div class="form mt-5">
            ...
            <div class="mt-3 d-flex">
              <div class="p-field-radiobutton d-flex">
                <RadioButton id="type1" v-model="state.postData.type" :value="1" name="type"/>
                <label class="mx-2" for="type1">{{ $t("place") }}</label>
              </div>
              <div class="p-field-radiobutton d-flex">
                <RadioButton id="type2" v-model="state.postData.type" :value="2" name="type"/>
                <label class="mx-2" for="type2">{{ $t("desk") }}</label>
              </div>
              <div class="p-field-radiobutton d-flex">
                <RadioButton id="type3" v-model="state.postData.type" :value="3" name="type"/>
                <label class="mx-2" for="type3">{{ $t("chair") }}</label>
              </div>
              <div class="p-field-radiobutton d-flex">
                <RadioButton v-if="state.postData.type == 4" id="type4" v-model="state.postData.type" :value="4"
                             name="type"/>
                <RadioButton v-if="state.postData.type == 5 || state.postData.type !== 4" id="type4"
                             v-model="state.postData.type"
                             :value="5"
                             name="type"/>
            ...
            <template v-if="state.postData.type === 4 || state.postData.type === 5">
              <div class="d-flex flex-column smart-text-align mb-2">
                <Dropdown v-model="state.vehicleType" :options="state.vehicles" optionLabel="name"
                          panelClass="smart-text-align" @change="state.changeVehicle()"/>
              </div>
              <div class="d-flex flex-column smart-text-align mb-2">
                <label for="title">{{ $t("vehicleName") }}</label>
                <InputText id="vehicleName" v-model="state.postData.title"
                           :class="!state.validation ? 'p-invalid':''" :placeholder="$t('vehicleName')"
                           class="input-control"
                           type="text"/>
              </div>
            ...
            <div class="card">
              ...
              <SelectImageWorkPage :data="imageTabData" :type="state.postData.type"
...
import {createStationPageState} from "./createStationPageState";
...
export default defineComponent({
  ...
  emits: ['sendData'],
  setup(props, {emit}) {
    const state = ref(new createStationPageState());
    ...
    async function callbackImage(event) {
      ...
      let list2 = [];
      event.list.docPhotoList.forEach(item => {
        const image = {
          base64: item.file,
      ...
      state.value.station.docImages = list2;
