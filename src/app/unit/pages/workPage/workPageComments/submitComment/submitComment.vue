  ...
  >
    ...
    <div style=" margin-bottom: 83px;">
      ...
      <div v-if="state.faceScore!=4" class="p-0 m-0">
        ...
        <div v-if="showPrivate==false" class="range-block  mt-4 text-center">
          ...
          <div class="input-range-main mt-4">
            <input id="mainSlider" v-model="mainSlider" class="slider margin-auto w-100" max="5" min="0" step="0.01"
                   type="range" @change="changeMainSlider"
        ...
        <div v-if="showPrivate==true" class="range-block  mt-4 text-center">
          <div class="d-flex margin-auto w-50 text-center" @click="showPrivate= !showPrivate">
          ...
          <div class="input-range-main mb-2 mt-3">
            <input id="weakSlider" v-model="state.scoreItem1" class="slider margin-auto w-100" max="5" min="0"
                   step="0.01"
                   type="range" @input="onRangeChange('weakSlider')">

          </div>

          <div class="text-center mb-2 mt-3">{{ $t('good') }}</div>
          <div class="input-range-main mt-1">
            <input id="good1Slider" v-model="state.scoreItem2" class="slider margin-auto w-100" max="5" min="0"
                   step="0.01"
                   type="range" @input="onRangeChange('good1Slider')">

          </div>

          <div class="text-center mb-2 mt-3">{{ $t('good') }}</div>
          <div class="input-range-main mt-1">
            <input id="good2Slider" v-model="state.scoreItem3" class="slider margin-auto w-100" max="5" min="0"
                   step="0.01"
                   type="range" @input="onRangeChange('good2Slider')">

          </div>

          <div class="text-center mb-2 mt-3">{{ $t('soExcellent') }}</div>
          <div class="input-range-main mt-1">
            <input id="soExcellentSlider" v-model="state.scoreItem4" class="slider margin-auto w-100" max="5" min="0"
                   step="0.01"
                   type="range" @input="onRangeChange('soExcellentSlider')">

          </div>

          <div class="text-center mb-2 mt-3">{{ $t('excellent') }}</div>
          <div class="input-range-main mt-1">
            <input id="excellentSlider" v-model="state.scoreItem5" class="slider margin-auto w-100" max="5" min="0"
                   step="0.01"
                   type="range" @input="onRangeChange('excellentSlider')">
      ...
      <div class="text-center mt-4">
        <div class=" d-flex justify-content-center  align-content-center   mx-auto rtl" style="width: 96%">
          <button :class="state.faceScore==1?'selected':''"
                  class="button button-right py-2 width33 presenceBtn" type="button" @click="state.faceScore=1">
            <i class="far fa-smile-beam icon-size-23 face"></i>
            <div>{{ $t("Isuggest") }}</div>
          </button>
          <button :class="state.faceScore==2?'selected':''"
                  class="button button-left py-2 width33 presenceBtn" type="button" @click="state.faceScore=2">
            <i class="far fa-meh icon-size-23 face"></i>
            <div>{{ $t("noIdea") }}</div>
          </button>
          <button :class="state.faceScore==3?'selected':''"
                  class="button button-left py-2 width33 presenceBtn" type="button" @click="state.faceScore=3">
            <i class="far fa-frown icon-size-23 face"></i>
            <div>{{ $t("noIdea") }}</div>
          </button>
          <button :class="state.faceScore==4?'selected':''"
                  class="button button-left py-2 width33 presenceBtn" type="button" @click="state.faceScore=4">
            <i class="fas fa-question icon-size-23 face"></i>
            <div>{{ $t("iHaveQuestion") }}</div>
          </button>
      ...
      <div v-if="state.faceScore!=4" class="p-0 m-0">
        ...
        <div class="width-96  mt-4 text-center">
          ...
          <div class="d-flex mt-1 justify-content-between">
            <input v-model="state.positiveView" class="form-control cus-input">
            <div class="grey-cell" @click="state.addCommentPos(state.positiveView),state.positiveView=''"><i
                class="fas fa-plus "></i></div>
          </div>

        </div>

        <div v-for="(pv,index) in state.positiveViews " class="width-96  mt-4 text-center">
          <div class="d-flex mt-1 justify-content-between">
            <input v-model="pv.value" class="form-control  cus-input" readonly type="text">
            <div class="grey-cell" @click="state.deleteCommentPos(index)"><i class="far fa-trash-alt"></i></div>
          </div>
        ...
        <div class="width-96  mt-4 text-center">
          ...
          <div class="d-flex mt-1 justify-content-between">
            <input v-model="state.negativeView" class="form-control cus-input">
            <div class="grey-cell" @click="state.addCommentNeg(state.negativeView),state.negativeView=''"><i
                class="fas fa-plus "></i></div>
          </div>

        </div>

        <div v-for="(pv,index) in state.negativeViews " class="width-96  mt-4 text-center">
          <div class="d-flex mt-1 justify-content-between">
            <input v-model="pv.value" class="form-control  cus-input" readonly type="text">
            <div class="grey-cell" @click="state.deleteCommentNeg(index)"><i class="far fa-trash-alt"></i></div>
          </div>
...
export default {
  ...
  setup() {
    ...
    function onRangeChange(id, allowed = false) {
      ...
      let val = (mainRangeSlider.value - mainRangeSlider.getAttribute('min')) / (mainRangeSlider.getAttribute('max')) - (mainRangeSlider.getAttribute('min'))
...
input[type=range] {
  ...
  border-radius: 8px;
  height: 8px;
  background-image: -webkit-gradient(linear, right top, left top, color-stop(0.5, #f5c434), color-stop(0.5, #c7c7c7));
}
