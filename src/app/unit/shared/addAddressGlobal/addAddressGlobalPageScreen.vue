<template>
    <div>
        ...
        <Footer>
            <Button :loading="state.loading" class="footer-btn" :label="$t('submit')" @click="state.onSend()"/>
        </Footer>
    </div>
</template>
...
import {addAddressGlobalPageState} from './addAddressGlobalPageState';
import { ref } from 'vue';
...
export default {
    ...
    setup () {
        const state = ref(new addAddressGlobalPageState())
