<template>
  <div class="datePicker">
    <div  @click="datePickerHandleShow(true, 'opened')">
      <input
          ref="inputRef"
          id="InputDateId"
          v-model="state.date"
...
export default {
  name: "DatePicker",
  props: ["name", "inline", "format", "placeholder", "showCurrentDate"],
  setup(props, context) {
    ...
    const state = reactive({
      ...
      date: props.showCurrentDate != null && props.showCurrentDate == false ? null : new PersianDate([jalal.year(), jalal.month(), jalal.date()])
          .toLocale("en")
          .format("YYYY/MM/DD")
    });
    onMounted(() => {
    ...
    const scrollToElementD = (parent, toEl) => {
      var topPos = document.queryeslector(toEl).offsetTop;
      document.queryeslectorAll(parent)[0].scrollTop = topPos - 10;
    };
