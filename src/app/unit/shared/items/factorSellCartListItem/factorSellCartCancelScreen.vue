  ...
  <div>
    <div class="list-controller" >
      <div class="item" v-for="item in list" :key="item">
        <div class="head" @click="(item.factorStaus !== 6 && item.factorStaus !== 5 && item.factorStaus !== 12) ? $function.goTo(`/factorDetails/${item.factorId}`) : null">
          ...
          <div class="controller">
            <div class="child text-left">
              <span>{{$filters.jalali(item.createdAt , true)}}</span>
              <span  >{{`UF_${item.factorId}`}}</span>
              <span style="width: 250px;">{{`${$t('totalPrice')} : ${$filters.currency(item.finalPrice , item.currencyId == 0)}`}} <currency :currencyId="item.currencyId" /></span>
            </div>
          </div>
        </div>
        <div class="footer" @click="(item.factorStaus !== 6 && item.factorStaus !== 5 && item.factorStaus !== 12) ? $function.goTo(`/factorDetails/${item.factorId}`) : $function.goTo(`/customMadeProduct/${item.buyerPageId}`)">
          <div class="child w-50">
            <div class="text-right">
              <span>{{$t('shoppingType')}} : {{checkProduct(item.productType)}}</span>
            </div>
          </div>
           <div class="child w-50">
              <swiper :direction="'horizontal'" :free-mode="true" :scrollbar="{ el: '.swiper-scrollbar', draggable: true }"
                    ...
                    class="slider">
              <swiper-slide v-for="image in item.imageUrls" :key="image" class="pt-2 mt-1" style="cursor: pointer">
          ...
          <div class="child">
            <div class="text-right">
              <span>{{$t('sendType')}} : {{sellerTransferCheck(item.sellerAddressType)}}</span>
            </div>
          </div>
          <div class="child">
            <div class="text-right">
              <span>{{$t('dateOfCancellationFactor')}} : {{$filters.jalali(item.modifiedAt , true)}}</span>
            </div>
          </div>
          <div class="child">
            <div class="text-right">
              <span>{{$t('reasonOfCancellationFactor')}} : {{item.cancelDescription}}</span>
            </div>
          </div>
        </div>
      </div>
