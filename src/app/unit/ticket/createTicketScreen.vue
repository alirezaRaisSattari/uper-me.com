<template>
  <div>
    ...
    <div class="mx-2">
      ...
      <select v-model="state.ticketUnit" class="form-control px-2 my-3">
        <option
          v-for="units in unitsList"
          :key="units.key"
          :value="units.key"
          class="my-2"
          @click="sendValue(units.key)"
        >
          {{ units.title }}
        </option>
      </select>
      ...
      <Form v-slot="{ errors }" :validation-schema="loginSchema">
        <Field
          ...
          class="form-control py-4 px-2 mt-4"
          name="ticketSubject"
          type="text"
        />
        <div class="text-danger text-sm">{{ errors.ticketSubject }}</div>
        <Field
    ...
    <div class="row d-flex justify-content-center py-2 mx-2">
      <button
        class="btn btn-light w-25 submit-button header-shadow mb-5 mx-2"
        type="button"
        @click="submitTicket()"
      >
        {{ $t("submit") }}
...
export default {
  ...
  setup() {
    const state = ref(new CreateTicketState());
    ...
    function importanceSubject(id) {
      state.value.ticketPriority = id;
    }
    ...
    function submitTicket() {
      ...
      if (state.value.ticketSubject.length < 3) {
        ToastService.getInstance().error(i18n.global.tc("chooseticketSubject"));
        return;
      }
